...
export class EnquiryController {
    ...
    private buildAdminMail(data: EnquiryPayload) {
        return {
    ...
    private buildAutoReply(data: EnquiryPayload) {
    ...
    async submitEnquiry(@Body() body: EnquiryPayload) {
        ...
        try {
            await this.transporter.sendMail(this.buildAdminMail(body));

            const autoReply = this.buildAutoReply(body);
            if (autoReply) {
            ...
            throw new HttpException('Failed to send email. Please try again.', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
}
